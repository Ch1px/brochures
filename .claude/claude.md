...
The editor wraps `BrochureEditor` in a Liveblocks `<RoomProvider id={"brochure-" + _id}>` so peer admins viewing the same brochure show up as avatars in the topbar (Tier 1), as live cursors over the preview frame, and as coloured outlines on whichever section they currently have selected (Tier 2). Identity (name, avatar URL, deterministic colour) is baked into the Liveblocks token by `/api/liveblocks-auth` from the validated Clerk session — peers can't spoof identity by mutating presence.

The `Presence` shape in `src/lib/liveblocks-shared.ts` carries `cursor: { x, y } | null` (normalised 0-1 against `.preview-stage-frame`'s *scroll-content* dimensions, so peers with different viewport sizes still render at the same semantic position — and a peer hovering over a section that's offscreen for the local user keeps their cursor attached to that section instead of floating in the wrong place) and `selectedSectionKey: string | null` (broadcast when the local user clicks into a section, so peers can decorate the same section in their own view).

Cursor presence and selection presence are deliberately consumed through different paths to avoid expensive re-renders. `OthersCursors` (rendered as a child of `.preview-stage-frame`) reads `useOthers()` directly and re-renders on every cursor update — but it's a small absolute-positioned overlay, so the cost is local. Selection presence is projected through `PeerPresenceProvider` using `useOthersMapped` with a custom equality check that ignores `cursor` changes; the resulting `PeerSelections` map is exposed via `PeerPresenceContext` so `PagesPanel` (coloured dot next to the section row) and `PreviewStage` (CSS variable + data attribute on the hitbox, used by an outline rule) only re-render when an actual selection changes — *not* on every peer mousemove.

**Why no name pill on the peer-selection outline.** `.preview-section-hitbox` uses `display: contents` so it has no box of its own — an absolutely-positioned overlay inside it would size to `.brochure-page` (the next positioned ancestor), not to the section. The local `.selected` pattern works around this by applying `outline` directly to the inner `<section>` element via a descendant CSS rule. Tier 2 mirrors that for peer outlines (`.preview-section-hitbox[data-peer-selected] > section.section { outline: ... var(--peer-color) }`). Adding a name pill would need a portal or a JS-measured fixed-position element; for our 3-admin scale, the avatar's coloured ring in the topbar plus the `PagesPanelPeerDot` is enough disambiguation. If we ever want the pill, the cleanest path is `createPortal` into the `<section>` element via a ref captured in `SectionRenderer`.
